        ...
        <div className="mt-3 space-y-2">
          {(pending || []).map((pattern) => (
            <div
              ...
              className="rounded-xl border border-blinkit-border px-3 py-2 flex items-center justify-between gap-3"
            >
              <div>
                <p className="text-sm font-semibold text-blinkit-dark">
                  {pattern.productName || pattern.productId?.name || "Product"}
                </p>
                <p className="text-xs text-blinkit-gray">
                  Suggested qty: {pattern.preferredQuantity} | Confidence:{" "}
                  {Math.round((pattern.confidenceScore || 0) * 100)}%
                </p>
              </div>
          ...
          {!loading && pending.length === 0 && (
            <p className="text-sm text-blinkit-gray">No pending reorders.</p>
          )}
        </div>
      </section>
      ...
      <section className="mt-5 rounded-2xl border border-blinkit-border bg-white p-4">
        ...
        <div className="mt-3 space-y-2">
          {(patterns || []).map((pattern) => (
            ...
            >
              <div>
                ...
                <p className="text-xs text-blinkit-gray">
                  Every {Math.round(pattern.avgDaysBetweenOrders || 0) || "-"} days | Confidence{" "}
                  {Math.round((pattern.confidenceScore || 0) * 100)}%
                </p>
              </div>
              <button
          ...
          {!loading && patterns.length === 0 && (
            <p className="text-sm text-blinkit-gray">No learned patterns yet.</p>
          )}
        </div>
      </section>
      ...
      <section className="mt-5 rounded-2xl border border-blinkit-border bg-white p-4">
        ...
        <div className="mt-3 space-y-2">
              ...
              <div>
                ...
                <p className="text-xs text-blinkit-gray">
                  ...
                  {pattern.nextPredictedDate
                    ? new Date(pattern.nextPredictedDate).toLocaleDateString()
                    : "N/A"}
                </p>
              </div>
              <button
          ...
          {!loading && upcoming.length === 0 && (
            <p className="text-sm text-blinkit-gray">No upcoming reorders.</p>
          )}
        </div>
      </section>
