...
const PantryPage = () => {
  ...
  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      const categoryMatch = activeCategory === "all" || (item.category || "other") === activeCategory;
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-blinkit-gray mt-1">{items.length} items • {totalQty} total quantity</p>
          </div>
          <input
            value={search}
        ...
        {expiring.length > 0 && (
          <section className="mt-5 rounded-2xl border border-red-200 bg-red-50 p-4">
            <p className="text-sm font-semibold text-red-700">{expiring.length} items expiring this week</p>
          </section>
        )}
        ...
        <section className="mt-5 flex gap-2 overflow-x-auto no-scrollbar">
          {categories.map((cat) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-semibold border ${activeCategory === cat ? "bg-blinkit-green text-white border-blinkit-green" : "bg-white border-blinkit-border text-blinkit-dark"}`}
            >
              {cat}
            </button>
          ))}
        ...
        <section className="mt-5 rounded-2xl border border-blinkit-border bg-white p-4">
          ...
          <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            {!loading && expiring.length === 0 && <p className="text-sm text-blinkit-gray">No items expiring soon.</p>}
          </div>
        </section>
        ...
        <section className="mt-5 rounded-2xl border border-blinkit-border bg-white p-4">
          ...
          <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
            {(filteredItems || []).map((item) => (
              <div key={item._id || item.id} className="rounded-xl border border-blinkit-border px-3 py-2 flex items-center justify-between gap-2">
                <div>
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-semibold text-blinkit-dark">{item.name}</p>
                    <ExpiryChip expiryDate={item.expiryDate} />
                  </div>
                  <p className="text-xs text-blinkit-gray">{item.quantity} {item.unit}</p>
                </div>
                <div className="flex items-center gap-2">
                  <button
            ...
            {!loading && filteredItems.length === 0 && <p className="text-sm text-blinkit-gray">No pantry items yet.</p>}
          </div>
        </section>
      </main>
      <Footer />
